export function SimulationHero() {
  return (
    <div className="w-full h-96 flex flex-row justify-between items-center mx-auto px-4">
      <h3 className="text-theme-grayscale-black text-2xl font-bold font-title mb-4">
        Simulação de Projetos Ágeis
      </h3>
      <p className="text-theme-grayscale-darkest text-start font-sans mb-6 w-1/2">
        Lorem ipsum dolor, sit amet consectetur adipisicing elit. Nesciunt sed
        magnam vel ex earum, aliquam voluptas asperiores dolores esse
        exercitationem sapiente quae et voluptatem rem, nisi suscipit enim
        officiis eum. Lorem, ipsum dolor sit amet consectetur adipisicing elit.
        Maxime ratione sed, ut asperiores repellat maiores delectus ducimus
        eligendi praesentium aperiam eveniet suscipit neque, expedita ipsa
        tempore, possimus quidem provident voluptatum! Lorem ipsum dolor sit
        amet consectetur adipisicing elit. Enim eius voluptate perspiciatis aut
        pariatur officiis, maiores molestias a assumenda expedita tempora!
        Molestias quae rem veniam deleniti repellendus unde doloribus beatae?
      </p>
    </div>
  );
}
