...
export function HeroCreators({ content }: HeroCreatorsProps) {
  ...
  return (
    <div className="w-full h-full  flex flex-col lg:flex-col justify-center px-8 pt-20 md:pt-40 md:pb-20 items-center bg-[#FCCF5C]">
      <div className="flex flex-col md:flex-row md:justify-center md:items-center lg:items-start gap-2 bg-purple-400">
        <Image
          className="h-[250px] w-[300px] md:h-[280px] md:w-[320px] lg:h-[450px] lg:w-[600px] md:block hidden bg-red-500"
          src={ibson}
        ...
        />
        <div className=" h-full w-full lg:w-1/2 flex flex-col md:flex-row lg:flex-col justify-between items-start bg-pink-400">
          <div>
      ...
      <div className=" flex-col justify-start items-start text-start font-sans text-base font-normal md:block lg:hidden w-full max-w-[625px] bg-orange-300">
        <p className="mb-6 ">{data.creatorsHeroFirstParagraph}</p>
        <p>{data.creatorsHeroSecondParagraph}</p>
        <div className="flex flex-row text-start items-start my-10 bg-red-500">
          <Image src={iconLinkedin} alt="icone linkedin" />
          <Link
            href="https://www.linkedin.com/in/ibson-cabral"
            target="_blank"
